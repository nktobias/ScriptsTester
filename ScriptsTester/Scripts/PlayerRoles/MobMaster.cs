...
namespace CalExtension.PlayerRoles
{
  public class MobMaster : PlayerRole
  {
    ...
    public class MobKillInfo
    {
      ...
      {
        if (Game.Debug)
          Game.PrintMessage("Mob_Changed: " + Game.RenameSufix(this.Mob.Name) + (String.Format("{0:n1}", (DateTime.Now - LastChangedTime).TotalMilliseconds)));
      ...
      {
        get { return Mob.Exist && Mob.Hits > 0 && Mob.Distance < 30; }
      }
        ...
        foreach (UOItem item in World.Player.Backpack.AllItems)
        {
          if (Array.IndexOf(ItemLibrary.ShrinkMountTypes.GraphicArray, item.Graphic) > -1 || 
            Array.IndexOf(ItemLibrary.ShrinkPackTypes.GraphicArray, item.Graphic) > -1 ||
              item.Graphic == 0x2119 && (item.Color == 0x0BB5 || item.Color == 0x0530)//baset
            )
...
//Serial: 0x4031939E  Name: "Claws of Destruction (Magni)"  Position: 73.3.0  Flags: 0x0000  Color: 0x0BB5  Graphic: 0x2119  Amount: 1  Layer: None Container: 0x4037F05A
    ...
    public UOItem NajdiKlamak()
    {
      ...
      else
      {
        foreach (Graphic klamak in ItemLibrary.ShrinkKlamaciArray)
        {
          ...
          if (itemKlamak.Exist && itemKlamak.Graphic != 0x2119 && (itemKlamak.Color != 0x0BB5 || itemKlamak.Color != 0x0530))
            break;
        }
      }
    ...
    //---------------------------------------------------------------------------------------------
    private List<Serial> deny = new List<Serial>();
    public void ShrinkniKlamaky()
    {
      Game.PrintMessage("ShrinkniKlamaky ...");
      ...
      if (shrinkKad.Exist)
      {
        ...
        var arr = characters.OrderBy(i => i.Distance).ToArray();

        Journal.Clear();
        foreach (UOCharacter klamak in arr)
        {
          bool success = false;
          bool used = false;
          string name = klamak.Name;

          if (klamak.Serial == World.Player.Serial || deny.Contains(klamak.Serial))
            continue;
          ...
          if (klamak.Distance <= 3)
          {
            ...
            success = !Journal.WaitForText(true, 200, "Ale co to delas?");

            if (!success)
              deny.Add(klamak.Serial);
          }

          if (used)
          {
            World.Player.PrintMessage("Shrink [" + name + "]" );
            break;
          }
        }
    ...
    {
      Game.CurrentGame.CurrentPlayer.GetPlayerRoleInstance<MobMaster>().ShrinkniKlamaky();
    }
