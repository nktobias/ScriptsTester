...
namespace CalExtension.PlayerRoles
{
  ...
  //  Serial: 0x401B7C69  Name: "Inscribed Staff of Frenzy"  Position: 0.0.0  Flags: 0x0000  Color: 0x0B16  Graphic: 0x26C9  Amount: 0  Layer: LeftHand Container: 0x00002965
    ...
    public void SwitchWeapon(params string[] filter)
    {
      ...
      {
        if (World.Player.CurrentWeapon().IsItemType(ItemLibrary.MytherilScimitar))
        {
          World.Player.PrintMessage("[Nelze MythScim!]", MessageType.Error);
          return;
        }
        else if (World.Player.CurrentWeapon().IsItemType(ItemLibrary.OakwoodQuarterStaff))
        {
          World.Player.PrintMessage("[Nelze QuarterStaff!]", MessageType.Error);
          return;
        }

        //Debug
        //UOItemType ls = new UOItemType() { Graphic = 0x0F60, Color = 0x0498 };

        //if (World.Player.CurrentWeapon().IsItemType(ls))
        //{
        //  World.Player.PrintMessage("[Nelze QuarterStaff!]", MessageType.Error);
        //  return;
        //}
        //  Serial: 0x403870A6  Name: "Longsword of Sunrise"  Position: 0.0.0  Flags: 0x0000  Color: 0x0498  Graphic: 0x0F60  Amount: 0  Layer: RightHand Container: 0x00364D66
    ...
    public void SwitchShield()
    {
      if (World.Player.CurrentWeapon().Exist)
      {
        if (World.Player.CurrentWeapon().IsItemType(ItemLibrary.OakwoodQuarterStaff))
        {
          World.Player.PrintMessage("[Nelze QuarterStaff!]", MessageType.Error);
          return;
        }
