...
namespace CalExtension.UOExtensions
{
  public class Targeting : PlayerRole
  {
   ...
   !Characters.IsSummon(i)).OrderBy(i => i.GetDistance(center)).ThenBy(i => i.Hits).ToList());
    ...
    public static Serial ParseTargets(params string[] targets)
    {
      ...
      if (targets != null)
      {
        ...
        {
          Serial serial = Targeting.ParseTarget(target);
          UOObject o = new UOObject(serial);

          if (serial.IsValid && o.Exist && o.Distance < 28)
          {
    ...
    private static Serial ParseTarget(string targetName)
    {
    ...
    private static Serial ParseTarget(string targetName, Serial defaultValue)
    {
      if (targetName != null)
      {
