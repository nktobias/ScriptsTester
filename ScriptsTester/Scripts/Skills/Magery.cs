...
namespace CalExtension.Skills
{
  ...
  public class Magery : Skill
  {
              ...
              if (!CastingSpellInfo.Silence)
              {
                ...
                if (CastingSpellInfo.Spell == StandardSpell.SummonCreature)
                {
                  if (!String.IsNullOrEmpty(Magery.CurrentCastingSummon))
                    message = Magery.CurrentCastingSummon + String.Empty + " " + World.Player.Mana + "";
                  else
                    message = GetSpellShorcut(CastingSpellInfo.Spell) + String.Empty + " " + World.Player.Mana + "";

                  Magery.CurrentCastingSummon = String.Empty;
                }
                else
                  message = GetSpellShorcut(CastingSpellInfo.Spell) + String.Empty + (CastingSpellInfo.IsScrool ? " " + World.Player.FindType(Magery.SpellScrool[CastingSpellInfo.Spell]).Amount + "" : " " + World.Player.Mana + "");
    ...
    public CastResultInfo CastSpell(StandardSpell spell, Serial target, bool useScrool, bool useSwitchHeadScrool)
    {
    ...
    public CastResultInfo CastSpell(StandardSpell spell, Serial target, bool useScrool, bool useSwitchHeadScrool, bool silence)
    {
    ...
    public void CastSummonCreature(string summonName, Serial target)
    {
      Game.RunScript(5000);
      Targeting.ResetTarget();
      ...
      if (!target.IsValid || !new UOObject(target).Exist || new UOObject(target).Distance > 25)
