...
namespace CalExtension.UI.Status
{
  ...
  public class StatusFormWrapper : InGameWindow
  {
    ...
    private void StatusBar_OnStatusShow(object sender, EventArgs e)
    {
      RefreshForms();
    }
...
//        IEnumerable<StatusForm> statusForms = defaultManager.OwnedWindows.OfType<StatusForm>();
    ...
    private int[]  posibleMaxs = new int[] { 12, 6, 24 };
    ...
    private void CbxMaxBars_SelectedValueChanged(object sender, EventArgs e)
    {
      ...
      {
        int max = 12;
        if (!Int32.TryParse(this.cbxMaxBars.SelectedItem + String.Empty, out max))
          max = 12;
        this.MaxBars = max;
      }
    }
